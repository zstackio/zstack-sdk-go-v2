...
func (cli *ZSClient) DeletePortForwardingRule(uuid string, deleteMode param.DeleteMode) error {
	return cli.DeleteWithSpec("v1/port-forwarding", fmt.Sprintf(\"%s\", uuid), string(deleteMode))
}
...
func (cli *ZSClient) DetachPortForwardingRule(uuid string, deleteMode param.DeleteMode) error {
	return cli.DeleteWithSpec("v1/port-forwarding", fmt.Sprintf(\"%s/vm-instances/nics\", uuid), string(deleteMode))
}
