...
func TestGetBaremetalInstance(t *testing.T) {
	// First query to get a valid UUID
	queryParam := param.NewQueryParam()
	queryParam.Limit(1)
	list, err := accountLoginCli.QueryBaremetalInstance(&queryParam)
	if err != nil {
		t.Errorf("TestGetBaremetalInstance Query error: %v", err)
		return
	}
	if len(list) == 0 {
		t.Skip("No BaremetalInstance found to test Get")
		return
	}

	// Get by UUID
	result, err := accountLoginCli.GetBaremetalInstance(list[0].UUID)
	if err != nil {
		t.Errorf("TestGetBaremetalInstance error: %v", err)
		return
	}
	golog.Infof("GetBaremetalInstance result: %s", result.UUID)
}
