...
const (
	UserAgent = "zstack-sdk-go/202206"
...
func GetDefaultClient() *http.Client {
	return GetClient(true, DefaultHttpRequestTimeout*time.Second)
}
...
func ParseJSONResponse(reqBody string, resp *http.Response, err error, debug bool) (http.Header, jsonutils.JSONObject, error) {
	...
	if resp.StatusCode < 300 {
		...
		if jrbody2.Contains("cause") {
			cause, _ := jrbody2.Get("cause")
			if ecause := jsonutils.GetAnyString(cause, []string{"message", "detail", "details", "error_msg"}); len(ecause) > 0 {
				if ecause != ce.Details {
					ce.Causes = &ecause
				}
			}
		}
