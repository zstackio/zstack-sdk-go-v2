...
func (s *StructFieldInfo) updateTags(k, v string) {
	s.Tags[k] = v
}

func (s StructFieldInfo) deepCopy() *StructFieldInfo {
	scopy := StructFieldInfo{
		Ignore:         s.Ignore,
...
func ParseStructFieldJsonInfo(sf reflect.StructField) StructFieldInfo {
	return ParseFieldJsonInfo(sf.Name, sf.Tag)
}

func ParseFieldJsonInfo(name string, tag reflect.StructTag) StructFieldInfo {
...
func (info *StructFieldInfo) MarshalName() string {
	if len(info.Name) > 0 {
		return info.Name
	}
...
type sStructFieldInfoMap map[string]StructFieldInfo

func newStructFieldInfoMap(caps int) sStructFieldInfoMap {
	return make(map[string]StructFieldInfo, caps)
}
